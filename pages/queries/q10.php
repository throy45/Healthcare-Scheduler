<?php
require_once '../../database.php'; 
include '../header.php';

// Query all facilities to populate the dropdown menu
$facilities_query = $conn->query('SELECT FacilityID, Name FROM Facilities ORDER BY Name');
$facilities = $facilities_query->fetch_all(MYSQLI_ASSOC);

// Set the default value for the selected facility to be the first option in the dropdown
$selected_facility = $facilities[0]['FacilityID'];

// Check if the user has submitted a form
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Get the selected facility ID from the form data
    $selected_facility = $_POST['facility_id'];
    
    // Query the emails generated by the selected facility, ordered by date
    $statement = $conn->prepare('SELECT el.Date, el.EmployeeID, CONCAT(e.FName, " ", e.LName) AS FullName, el.Subject, el.Body
                            FROM EmailLog el, Employees e
                            WHERE e.EmployeeID = el.EmployeeID AND el.FacilityID = ?
                            ORDER BY el.Date ASC;
    ');
    $statement->bind_param('i', $selected_facility);
    $statement->execute();
    $results = $statement->get_result()->fetch_all(MYSQLI_ASSOC);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../../pages/style.css">
    <title>Query 10</title>
</head>
<body>
    <h1>Emails by facility</h1>
    <form method="post">
        <label for="facility_id">Select a facility:</label>
        <select id="facility_id" name="facility_id">
            <?php foreach ($facilities as $facility) { ?>
                <option value="<?php echo $facility['FacilityID']; ?>"<?php if ($selected_facility == $facility['FacilityID']) { echo ' selected'; } ?>><?php echo $facility['Name']; ?></option>
            <?php } ?>
        </select>
        <button type="submit">View Emails</button>
    </form>

    <?php if (isset($results)) { ?>
        <table>
            <thead>
                <tr>
                    <th>EmployeeID</th>
                    <th>Employee Name</th>
                    <th>Date</th>
                    <th>Subject</th>
                    <th>Body</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($results as $row) { ?>
                    <tr>
                        <td><?php echo $row['EmployeeID']; ?></td>
                        <td><?php echo $row['FullName']; ?></td>
                        <td><?php echo $row['Date']; ?></td>
                        <td><?php echo $row['Subject']; ?></td>
                        <td><?php echo $row['Body']; ?></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
    <?php } ?>

    <a href="../../index.php">Back to main page</a>
</body>
</html>
